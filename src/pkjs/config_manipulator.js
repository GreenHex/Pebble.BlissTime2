...
//  var HTML = minified.HTML;
  ...
  var toggle_visibility = function() {
    ...
    if ( this.get() == '1' ) { // weather
      showItems = [ 'TEMPERATURE_UNITS', 'OWM_API_KEY', 'STATUS_UPDATE_INTERVAL', 'STATUS_UPDATE_START_TIME', 'STATUS_UPDATE_END_TIME' ];
      hideItems = [ 'STOCK_CODE' ];
    } else if ( this.get() == '2' ) { // stocks
      showItems = [ 'STOCK_CODE', 'STATUS_UPDATE_INTERVAL', 'STATUS_UPDATE_START_TIME', 'STATUS_UPDATE_END_TIME' ];
      hideItems = [ 'TEMPERATURE_UNITS', 'OWM_API_KEY' ];
    } else { // nothing
      showItems = [  ];
      hideItems = [ 'TEMPERATURE_UNITS', 'OWM_API_KEY', 'STOCK_CODE', 'STATUS_UPDATE_INTERVAL', 'STATUS_UPDATE_START_TIME', 'STATUS_UPDATE_END_TIME' ]; 
    }
    show( showItems );
    hide( hideItems );
  };
