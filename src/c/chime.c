...
void do_chime( struct tm *time, struct CONFIG_PARAMS *p_config_params ) {
  ...
  int mins_from_zero = time->tm_hour * 60 + time->tm_min + p_config_params->chime_offset;

  // Stop if buzzing is off
  if ( p_config_params->chime_interval == 0 ) return;
  
  // Stop if not on for the day
  if ( ! p_config_params->chime_on_days[time->tm_wday] ) return;
  
  // Stop if not within time range.
  // Add one minute to buzz_end to buzz on the last hour.
  if ( !is_X_in_range( p_config_params->chime_start_time * 60, p_config_params->chime_end_time * 60 + 1, mins_from_zero ) ) return;
  
  // Stop if not at offset
  if ( mins_from_zero % ( ( p_config_params->chime_interval == 1 ) ? 30 : 60 ) ) return;
  
  // is this half hour or full hour?
  if ( time->tm_min + p_config_params->chime_offset == 0 ) {
